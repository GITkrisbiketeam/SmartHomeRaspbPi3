...
import com.krisbiketeam.smarthomeraspbpi3.common.hardware.driver.SHT30
...
class HwUnitI2CTempRhSht30Sensor(
    name: String,
    location: String,
            ...
            job = launch(Dispatchers.IO) {
                ...
                while (isActive) {
                    ...
                    if (heatOnCounter++ >= heatOnTrigger) {
                        // wait for OneShotReadValue to close SHT30
                        delay(5000)
                        heatOnOff(true).also {
                            if (it.isFailure) {
    ...
    private suspend fun oneShotReadValue(): Result<HwUnitValue<TemperatureAndHumidity?>> {
        ...
        return withContext(Dispatchers.Main) {
            runCatching {
                hwUnitValue = HwUnitValue(SHT30(pinName).use {
    ...
    override suspend fun readValue(): Result<HwUnitValue<TemperatureAndHumidity?>> {
        ...
        return withContext(Dispatchers.Main) {
            runCatching {
                hwUnitValue = HwUnitValue(SHT30(pinName).use {
                    TemperatureAndHumidity(null, null)
                }, System.currentTimeMillis())
                hwUnitValue
            }
        }
